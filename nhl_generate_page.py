...
def compute_stats(logs, pos="F"):
    """Modele V7 : moyenne simple + filtre TOI + filtre regularite."""
    df      = pd.DataFrame(logs)
    n       = len(df)
    avg_pts = df["points"].sum() / n
...
def fetch_probable_goalies(game_id, home_abbrev, away_abbrev):
    try:
        r = requests.get("https://api-web.nhle.com/v1/gamecenter/{}/boxscore".format(game_id), timeout=10)
        r.raise_for_status()
        data    = r.json()
        home_id = data.get("homeTeam", {}).get("id")
        hg = ag = None
        for spot in data.get("rosterSpots", []):
            if spot.get("positionCode") != "G": continue
            name = "{} {}".format(spot.get("firstName",{}).get("default",""),
                                   spot.get("lastName", {}).get("default","")).strip()
            if spot.get("teamId") == home_id:
                if not hg: hg = name
            else:
                if not ag: ag = name
        if hg or ag: return {"home": hg, "away": ag}
    except Exception:
        pass
...
def build_goalie_context(games):
    ...
    for g in games:
        ...
        context[gid] = {
            ...
            "home_is_elite": hg in ELITE_GOALIES, "away_is_elite": ag in ELITE_GOALIES,
...
def fetch_odds():
    try:
        betclic_map, eu_map = {}, {}
        r = requests.get("https://api.the-odds-api.com/v4/sports/icehockey_nhl/odds/",
            params={"apiKey": ODDS_API_KEY, "regions": "eu", "markets": "player_points",
                    "oddsFormat": "decimal", "bookmakers": "betclic"}, timeout=15)
        if r.status_code == 200:
            for event in r.json():
                for bm in event.get("bookmakers", []):
                    for market in bm.get("markets", []):
                        ...
                        for o in market.get("outcomes", []):
                            if o.get("point", 0) != 0.5: continue
                            n, p = o.get("name","").lower().strip(), o.get("price", 0)
                            if n not in betclic_map or p > betclic_map[n]: betclic_map[n] = p
        r2 = requests.get("https://api.the-odds-api.com/v4/sports/icehockey_nhl/odds/",
            params={"apiKey": ODDS_API_KEY, "regions": "eu", "markets": "player_points",
                ...
                for bm in event.get("bookmakers", []):
                    for market in bm.get("markets", []):
                        ...
                        for o in market.get("outcomes", []):
                            if o.get("point", 0) != 0.5: continue
                            n, p = o.get("name","").lower().strip(), o.get("price", 0)
                            if n not in eu_map or p > eu_map[n]: eu_map[n] = p
        final = {}
        for name in set(betclic_map) | set(eu_map):
            final[name] = (betclic_map[name], "Betclic") if name in betclic_map else (eu_map[name], "EU")
        print("     {} cotes recuperees".format(len(final)))
        return final
    except Exception as e:
        print("     [Odds] erreur: {}".format(e))
        return {}
...
def build_html(games, top_players, history, generated_at, odds_map=None, value_picks=None):
    ...
    for i, p in enumerate(top_players):
        ...
        cote_val, _ = odds_map.get(p["name"].lower().strip(), (COTE_DEFAUT, ""))
        val_pct     = round(((p["p_over"] * cote_val) - 1) * 100, 1)
        cote_color  = "var(--gold)" if val_pct >= VALUE_MIN_PCT else "var(--muted)"
        cote_disp   = '<span style="color:{c};font-weight:600">{v}</span>'.format(c=cote_color, v=cote_val)
        rows_today += """
...
def main():
    ...
    top = []
    if games:
        # 4. Rosters + morning report
        print("\n[4/6] Morning report...")
        injured_ids, _ = fetch_injury_report()
        print("     {} blesse(s)".format(len(injured_ids)))
        goalie_context = build_goalie_context(games)
        print("     {}/{} gardiens confirmes".format(
            sum(1 for c in goalie_context.values() if c.get("home_goalie") != "TBD"), len(games)))

        team_to_game, team_is_home = {}, {}
        for g in games:
            gid = g.get("game_id")
        ...
        # Cotes + value picks
        print("     Cotes The Odds API...")
        odds_map = fetch_odds()
        value_picks = []
        for p in top:
            cote_val, cote_src = odds_map.get(p["name"].lower().strip(), (COTE_DEFAUT, "defaut"))
        ...
        print("     {} value pick(s)".format(len(value_picks)))
    ...
    if GITHUB_TOKEN:
        ...
        ok = github_put(HTML_FILE, build_html(games, top, history, generated_at, odds_map, value_picks),
                        "auto: update NHL page - {}".format(today_iso), sha)
        print("index.html pousse !" if ok else "Erreur push index.html")
    else:
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(build_html(games, top, history, generated_at, odds_map, value_picks))
