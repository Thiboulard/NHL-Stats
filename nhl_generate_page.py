...
def build_html(games, top_players, history, generated_at, odds_map=None, value_picks=None):
    ...
    for i, p in enumerate(top_players):
        ...
        glitch_cls = "glitch-on" if prob >= 70 else ""
        ...
        <tr style="animation:fadeIn 0.4s ease {delay}s both">
          ...
          <td>
            <div class="prob-bar-wrap">
              ...
              <span class="prob-pct {glitch_cls}" style="color:{pcolor}">{prob}%</span>
            </div>
          </td>
          <td>{cote}</td>
          <td>{signal}</td>
            ...
            pclass=prob_class, glitch_cls=glitch_cls, prob=prob, pcolor=pct_color,
  ...
  <style>
    ...
    *{{margin:0;padding:0;box-sizing:border-box;}}
    ...
    body::before{{
      ...
      background-image:radial-gradient(circle,rgba(255,213,0,0.055) 1px,transparent 1px);
    ...
    .header-top-label{{font-family:'Bangers',cursive;font-size:0.9rem;letter-spacing:0.15em;color:var(--yellow);}}
    .header-main{{
    ...
    .header-main::before{{
      ...
      background:repeating-linear-gradient(-45deg,transparent,transparent 10px,rgba(0,0,0,0.035) 10px,rgba(0,0,0,0.035) 20px);
      pointer-events:none;
    }}
    h1{{
    ...
    h1 .accent2{{color:var(--blue);}}

    /* GLITCH on h1 */
    @keyframes glitch-main{{
      0%,90%,100%{{
        text-shadow:5px 5px 0 var(--red),-3px -3px 0 rgba(37,99,255,0.5);
        transform:skewX(0deg);
      }}
      91%{{text-shadow:-6px 3px 0 var(--cyan),6px -3px 0 var(--red);transform:skewX(-3deg) translateX(3px);}}
      92%{{text-shadow:6px -3px 0 var(--magenta),-4px 3px 0 var(--blue);transform:skewX(3deg) translateX(-3px);}}
      93%{{text-shadow:0 0 0 transparent;transform:skewX(0);}}
      94%{{text-shadow:-5px 0 0 var(--cyan),5px 0 0 var(--red);transform:translateX(2px);}}
      95%{{text-shadow:5px 5px 0 var(--red),-3px -3px 0 rgba(37,99,255,0.5);transform:skewX(0);}}
    }}
    h1{{animation:glitch-main 7s ease-in-out infinite;}}

    .tagline{{font-family:'Bangers',cursive;font-size:1.1rem;letter-spacing:0.2em;color:var(--ink);margin-top:0.3rem;opacity:0.7;}}
    .meta-pill{{
      display:inline-block;
      background:var(--ink);color:var(--text);
      font-family:'Share Tech Mono',monospace;font-size:0.65rem;letter-spacing:0.12em;
    ...
    /* GLOBAL RATE */
    .global-rate{{
      margin:1rem auto 0;max-width:380px;
    ...
    .global-rate::before{{
      ...
      font-family:'Bangers',cursive;font-size:0.75rem;letter-spacing:0.15em;
      padding:0 0.5rem;border:3px solid var(--ink);
    }}
    ...
    .global-rate-value{{font-family:'Bangers',cursive;font-size:3.5rem;letter-spacing:0.05em;line-height:1;}}
    ...
    .ticker{{
      ...
      padding:0.45rem 0;position:relative;z-index:10;
    }}
    .ticker-inner{{display:flex;gap:0;animation:ticker 32s linear infinite;white-space:nowrap;}}
    @keyframes ticker{{from{{transform:translateX(0);}}to{{transform:translateX(-50%);}}}}
    .ticker-item{{
      font-family:'Bangers',cursive;font-size:0.85rem;letter-spacing:0.12em;
      color:var(--yellow);padding:0 2rem;border-right:2px solid rgba(255,213,0,0.25);
    }}
    ...
    .tabs{{
      ...
      border-bottom:4px solid var(--yellow);overflow-x:auto;
    }}
    .tab-btn{{
      background:transparent;border:none;
      border-right:2px solid rgba(255,213,0,0.15);
      padding:0.9rem 1.8rem;
      font-family:'Bangers',cursive;font-size:1rem;letter-spacing:0.15em;
      color:rgba(255,213,0,0.45);cursor:pointer;transition:all 0.15s;white-space:nowrap;position:relative;
    }}
    .tab-btn:hover{{color:var(--yellow);background:rgba(255,213,0,0.06);}}
    .tab-btn.active{{color:var(--ink);background:var(--yellow);}}
    ...
    /* ===== MAIN ===== */
    main{{position:relative;z-index:10;max-width:1140px;margin:0 auto;padding:2rem 1.5rem 4rem;}}

    .section-title{{
      font-family:'Bangers',cursive;font-size:1.6rem;letter-spacing:0.1em;
      color:var(--yellow);margin-bottom:1.2rem;margin-top:1.5rem;
      display:inline-flex;align-items:center;gap:0.75rem;
      background:var(--ink);padding:0.3rem 1.2rem 0.3rem 0.8rem;
      clip-path:polygon(0 0,calc(100% - 12px) 0,100% 50%,calc(100% - 12px) 100%,0 100%);
    }}

    .matchups-bar{{
      background:var(--surface);border:3px solid rgba(255,213,0,0.3);
      box-shadow:4px 4px 0 var(--ink);
      padding:0.6rem 1.1rem;margin-bottom:1.5rem;
      font-family:'Share Tech Mono',monospace;font-size:0.72rem;letter-spacing:0.06em;
      color:var(--text);
    }}
    .matchups-bar span{{color:var(--yellow);font-weight:700;margin-right:0.5rem;}}

    .no-games{{
      background:rgba(255,58,45,0.2);border:3px solid var(--red);
      box-shadow:4px 4px 0 var(--ink);padding:1rem;margin-bottom:1.5rem;
      text-align:center;color:var(--red);font-family:'Bangers',cursive;
    ...
    .table-wrap{{
      ...
      box-shadow:7px 7px 0 rgba(255,213,0,0.2);
    ...
    th{{
      ...
      font-family:'Bangers',cursive;letter-spacing:0.12em;
      color:var(--yellow);font-size:0.8rem;white-space:nowrap;
      border-right:1px solid rgba(255,213,0,0.12);
    }}
    tbody tr{{border-bottom:1px solid rgba(255,213,0,0.08);transition:background 0.1s;}}
    tbody tr:nth-child(even){{background:var(--surface2);}}
    tbody tr:hover{{background:rgba(255,213,0,0.08);}}
    td{{padding:0.6rem 0.9rem;color:var(--text);white-space:nowrap;font-weight:600;border-right:1px solid rgba(255,255,255,0.04);}}
    td.rank{{font-family:'Bangers',cursive;font-size:1.4rem;color:var(--red);width:40px;text-align:center;}}
    td.name{{color:#fff;font-weight:900;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.03em;}}
    td.team{{color:var(--cyan);font-family:'Bangers',cursive;font-size:0.9rem;letter-spacing:0.1em;}}

    /* PROB BAR */
    .prob-bar-wrap{{display:flex;align-items:center;gap:0.5rem;}}
    .prob-bar{{flex:1;height:8px;background:rgba(255,255,255,0.06);border:2px solid rgba(255,213,0,0.2);overflow:hidden;min-width:55px;}}
    ...
    .prob-low{{background:#555;}}
    .prob-pct{{font-family:'Bangers',cursive;font-size:0.9rem;min-width:38px;text-align:right;}}

    /* GLITCH on high prob % */
    @keyframes glitch-num{{
      0%,88%,100%{{text-shadow:none;transform:none;}}
      89%{{text-shadow:-3px 0 var(--cyan),3px 0 var(--red);transform:skewX(-4deg);}}
      90%{{text-shadow:3px 0 var(--magenta),-3px 0 var(--blue);transform:skewX(4deg);}}
      91%{{text-shadow:none;transform:none;}}
    }}
    .prob-pct.glitch-on{{animation:glitch-num 4s ease-in-out infinite;color:var(--cyan);}}

    /* SIGNAL TAGS */
    .over-tag{{
      display:inline-block;padding:0.1rem 0.45rem;
      font-family:'Bangers',cursive;font-size:0.75rem;letter-spacing:0.08em;
      border:2px solid rgba(255,255,255,0.2);
    }}
    .over-strong{{background:var(--blue);color:#fff;border-color:var(--blue);}}
    .over-mid{{background:var(--red);color:#fff;border-color:var(--red);}}

    /* HISTORY */
    .hist-tabs{{display:flex;flex-wrap:wrap;gap:0.4rem;margin-bottom:1.25rem;}}
    .hist-tab{{
      background:var(--surface);border:2px solid rgba(255,213,0,0.25);
      padding:0.35rem 0.8rem;font-family:'Bangers',cursive;font-size:0.85rem;
      letter-spacing:0.1em;color:rgba(255,213,0,0.55);cursor:pointer;transition:all 0.15s;
      display:inline-flex;align-items:center;gap:0.4rem;
    }}
    .hist-tab:hover,.hist-tab.active{{background:var(--yellow);color:var(--ink);border-color:var(--yellow);}}
    .tab-rate{{font-size:0.75rem;font-weight:700;}}
    .hist-panel-header{{
      display:flex;justify-content:space-between;align-items:center;
      margin-bottom:0.75rem;font-family:'Bangers',cursive;font-size:1rem;
      letter-spacing:0.08em;color:var(--yellow);
      padding:0.4rem 0.75rem;background:var(--ink);border:3px solid rgba(255,213,0,0.4);
    }}
    .panel-rate{{color:var(--cyan);}}
    .res-cell{{width:30px;text-align:center;}}
    .res-win{{display:inline-block;background:var(--blue);color:#fff;font-family:'Bangers',cursive;font-size:0.9rem;padding:0 5px;border:2px solid rgba(255,255,255,0.2);}}
    .res-loss{{display:inline-block;background:var(--red);color:#fff;font-family:'Bangers',cursive;font-size:0.9rem;padding:0 5px;border:2px solid rgba(255,255,255,0.2);}}
    .res-nogame{{color:var(--muted);font-family:'Bangers',cursive;}}
    .res-pending{{color:var(--yellow);font-family:'Bangers',cursive;}}

    /* COMBOS */
    .combo-intro{{font-size:0.75rem;color:var(--muted);margin-bottom:1.25rem;line-height:1.6;font-weight:600;letter-spacing:0.05em;}}
    .combo-tabs{{display:flex;gap:0;border-bottom:4px solid var(--yellow);margin-bottom:1.5rem;}}
    .combo-tab{{
      background:transparent;border:none;border-bottom:3px solid transparent;
      padding:0.6rem 1.4rem;font-family:'Bangers',cursive;font-size:1rem;
      letter-spacing:0.12em;color:rgba(255,213,0,0.45);cursor:pointer;transition:all 0.15s;
    }}
    .combo-tab:hover{{color:var(--yellow);}}
    .combo-tab.active{{color:var(--ink);background:var(--yellow);}}
    .combo-card{{
      background:var(--surface);border:3px solid rgba(255,213,0,0.25);
      box-shadow:5px 5px 0 var(--ink);padding:1.1rem 1.25rem;margin-bottom:1rem;
      transition:transform 0.15s,box-shadow 0.15s,border-color 0.15s;
    }}
    .combo-card:hover{{transform:translate(-2px,-2px);box-shadow:7px 7px 0 var(--ink);border-color:var(--yellow);}}
    .combo-header{{display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem;}}
    .combo-rank{{font-family:'Bangers',cursive;font-size:2rem;color:var(--red);line-height:1;}}
    ...
    .combo-cote strong{{color:var(--yellow);}}
    .combo-prob-bar{{height:6px;background:rgba(255,255,255,0.06);border:2px solid rgba(255,213,0,0.15);overflow:hidden;margin-bottom:0.85rem;}}
    ...
    .combo-player{{
      background:var(--ink);border:2px solid rgba(255,213,0,0.2);
      padding:0.3rem 0.65rem;display:flex;align-items:center;gap:0.5rem;
      clip-path:polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%);
    }}
    .combo-pname{{font-size:0.75rem;color:var(--yellow);font-weight:900;letter-spacing:0.04em;text-transform:uppercase;}}
    .combo-pteam{{font-size:0.62rem;color:var(--muted);letter-spacing:0.08em;}}
    .combo-pprob{{font-family:'Bangers',cursive;font-size:0.85rem;}}

    /* GLITCH on section titles — subtle */
    ...
    .section-title{{animation:glitch-title 8s ease-in-out infinite;}}

    /* GLITCH on value badge */
    @keyframes glitch-badge{{
      0%,85%,100%{{text-shadow:none;transform:none;filter:none;}}
      86%{{text-shadow:-2px 0 var(--cyan);transform:skewX(-2deg);}}
      87%{{text-shadow:2px 0 var(--red);transform:skewX(2deg);}}
      88%{{text-shadow:none;transform:none;}}
    }}
    .over-strong{{animation:glitch-badge 5s ease-in-out infinite;}}

    .glow-line{{
      height:4px;
      background:repeating-linear-gradient(90deg,var(--yellow) 0,var(--yellow) 12px,var(--ink) 12px,var(--ink) 16px);
      margin:2.5rem 0;opacity:0.5;
    }}

    footer{{
      position:relative;z-index:10;text-align:center;padding:1.5rem;
      border-top:4px solid var(--yellow);background:var(--ink);
      font-family:'Bangers',cursive;font-size:0.85rem;letter-spacing:0.12em;color:var(--muted);
    }}
...
<script>
  ...
  var comboColors = {{'combo-2':'var(--blue)','combo-3':'var(--red)','combo-4':'var(--magenta)'}};
  function showCombo(btn) {{
    document.querySelectorAll('.combo-tab').forEach(b => {{ b.classList.remove('active'); b.style.background='transparent'; b.style.color='rgba(255,213,0,0.45)'; }});
