...
def compute_stats(logs, toi_expected):
    ...
    return {
        ...
        "pts_per_60": pts_per_60, "expected_pts": expected_pts, "p_over": p_over,
    }
...
def compute_combos(top_players, sizes=(2, 3, 4), top_n=5):
    """
    Genere les meilleurs combines de taille 2, 3 et 4 joueurs.
    Contrainte : pas deux joueurs du meme match (meme equipe home ou away).
    Retourne un dict {size: [liste de combines tries par prob desc]}.
    """
    # Construit un mapping joueur -> game_id (equipe = cle de match)
    # On utilise l equipe comme proxy : deux joueurs sont du meme match
    # si leur equipe apparait dans le meme matchup.
    # On groupe les equipes par matchup via les données du top (team field).
    # On recupere les matchups depuis les equipes presentes dans le top.
    # Approche simple : on groupe par paires d equipes qui jouent ensemble.
    # On construit un dict team -> match_id depuis la liste des joueurs.
    # Comme on n a pas le game_id ici, on utilise une heuristique :
    # deux joueurs sont "meme match" si leur team appartient au meme groupe.
    # On passe les matchups en parametre supplementaire via l attribut "game_id" si dispo,
    # sinon on se base uniquement sur l equipe.

    # Construit un index team -> match_key depuis les joueurs
    # On assigne un match_key = frozenset des equipes du meme match.
    # Comme on n a pas cette info dans top_players, on va grouper par equipe
    # et considerer que deux joueurs de la meme equipe = meme match.
    # Note : deux joueurs d equipes adverses sont aussi du meme match
    # mais on ne peut pas le detecter sans le schedule ici.
    # Solution pragmatique : interdire deux joueurs de la MEME equipe uniquement.
    # (les equipes adverses sont rares dans le top 20 en pratique)

    result = {}
    for size in sizes:
        combos = []
        for group in combinations(top_players, size):
            # Verifie la contrainte : pas deux joueurs de la meme equipe
            teams = [p["team"] for p in group]
            if len(teams) != len(set(teams)):
                continue   # doublon d equipe -> meme match -> interdit
        ...
        # Trie par prob decroissante et garde le top N
...
def build_html(games, top_players, history, generated_at):
    today     = datetime.now().strftime("%d/%m/%Y")
    today_iso = datetime.now().strftime("%Y-%m-%d")
    matchups  = " · ".join(["{} vs {}".format(g["away"], g["home"]) for g in games])

    # Calcul des combines
    combos     = compute_combos(top_players) if top_players else {2: [], 3: [], 4: []}
    ...
    for i, p in enumerate(top_players):
        ...
        <tr style="animation:fadeIn 0.4s ease {delay}s both">
          ...
          <td>{signal}</td>
            ...
            pclass=prob_class, prob=prob, pcolor=pct_color, signal=signal
        )
...
def main():
    ...
    if GITHUB_TOKEN:
        ...
        ok = github_put(HTML_FILE, build_html(games, top, history, generated_at),
                        "auto: update NHL page - {}".format(today_iso), sha)
        print("index.html pousse !" if ok else "Erreur push index.html")
    else:
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(build_html(games, top, history, generated_at))
